...
class BusytexPipeline {
    ...
    async compile(files, main_tex_path, bibtex, verbose, driver, data_packages_js = []) {
        ...
        const synctex_path = tex_path.replace(/\.tex$/i, '.synctex.gz');

        const xetex = ['xelatex', '--no-shell-escape', '-synctex=1', '--interaction=batchmode', '--halt-on-error', '--no-pdf', '--fmt', this.fmt.xetex, tex_path].concat((this.verbose_args[verbose] || this.verbose_args[BusytexPipeline.VerboseSilent]).xetex);
        ...
        const pdftex_not_final = ['pdflatex', '--no-shell-escape', '-synctex=1', '--interaction=batchmode', '--halt-on-error', '--fmt', this.fmt.pdftex, tex_path].concat((this.verbose_args[verbose] || this.verbose_args[BusytexPipeline.VerboseSilent]).pdftex);

        const luahbtex = ['luahblatex', '--no-shell-escape', '-synctex=1', '--interaction=nonstopmode', '--halt-on-error', '--output-format=pdf', '--fmt', this.fmt.luahbtex, '--nosocket', tex_path].concat((this.verbose_args[verbose] || this.verbose_args[BusytexPipeline.VerboseSilent]).luahbtex);
        const luahbtex_not_final = ['luahblatex', '--no-shell-escape', '-synctex=1', '--interaction=nonstopmode', '--halt-on-error', '--no-pdf', '--fmt', this.fmt.luahbtex, '--nosocket', tex_path].concat((this.verbose_args[verbose] || this.verbose_args[BusytexPipeline.VerboseSilent]).luahbtex);

        const luatex = ['lualatex', '--no-shell-escape', '-synctex=1', '--interaction=nonstopmode', '--halt-on-error', '--output-format=pdf', '--fmt', this.fmt.luatex, '--nosocket', tex_path].concat((this.verbose_args[verbose] || this.verbose_args[BusytexPipeline.VerboseSilent]).luahbtex);
        const luatex_not_final = ['lualatex', '--no-shell-escape', '-synctex=1', '--interaction=nonstopmode', '--halt-on-error', '--no-pdf', '--fmt', this.fmt.luatex, '--nosocket', tex_path].concat((this.verbose_args[verbose] || this.verbose_args[BusytexPipeline.VerboseSilent]).luahbtex);
        ...
        const synctex = exit_code == 0 ? this.read_all_bytes(FS, synctex_path) : new Uint8Array(0);
        const bbl = this.read_all_text(FS, bbl_path);
        ...
        return { pdf: pdf, synctex: synctex, bbl: bbl, log: logcat, exit_code: exit_code, logs: logs };
    }
}
